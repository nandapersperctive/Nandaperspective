...
/* ── Hero stats: real counts from the content files, animated on view ── */
(function initHeroStats() {
    const statsWrap = document.getElementById("hero-stats");
    if (!statsWrap) return;

    const counts = {
        "stat-articles":   articleCatalog.length,
        "stat-stocks":     (typeof portfolioHoldings !== "undefined") ? portfolioHoldings.length : 0,
        "stat-categories": new Set(articleCatalog.map(a => a.categoryKey)).size
    };
    ...
    function run() {
        Object.entries(counts).forEach(([id, value]) => {
            ...
            if (!el) return;
            if (reduceMotion || value === 0) el.textContent = value;
